...
def replace_tags(filename_in, conversion_dict, exclude_empty=True):
    filename_out = filename_in.replace('.osm', '')
    filename_out = '%s-%s.osm' % (filename_out, 'tagged')
    ...
    for item in osm:
        ...
        if key not in conversion_dict:
            logger.warning('ssr:hovedgruppe = %s;ssr:type = %s not found in conversion table',
                           ssr_hovedgruppe, ssr_type)
            # fixme: add to not_found list?

        if key != '' and key in conversion_dict:
            tags = conversion_dict[key]
            if exclude_empty or len(tags) != 0:
            ...
            #print item, conversion_dict[key]
